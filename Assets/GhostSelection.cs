...
{
    string enemyTag = "EnemyGhost"; // tag to regroup enemy
    ...
    public float fireRate = 1f; // number of seconds between two shots
    ...
    void Shoot(Vector2 direction, float rotationZ)
    {
        ...
        else if (projectile.GetComponent<ProjectileAntiGhost>() != null)
        {
            projectile.GetComponent<ProjectileAntiGhost>().SetTarget(target);
        }
    }
    ...
    private void OnMouseUp()
    {
        if (enabled)
        {
            if (!displayRange)
            {
                ...
                {
                    if (turret.GetComponentInChildren<GhostSelection>() != null)
                    {
                        GhostSelection tmp = turret.GetComponentInChildren<GhostSelection>();
